...
export class SearchPage {
    ...
    private _totalPages: number | undefined = undefined;
    private _searchResults: SearchResult[] | undefined = undefined;
     ...
     */
    async getNextPage(): Promise<SearchPage> {
        if(this._isLast) {
     ...
     * @private
     */
    private async parseSearchPage(html?: string) {
        let rHtml: string = html ? html : await Fetcher.fetchSearchPage(this._name, this._pageIndex);
        const $ = cheerio.load(rHtml);

        const RESULT: SearchResult[] = new Array<SearchResult>();
