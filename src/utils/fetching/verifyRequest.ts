 ...
 */
export async function verifyRequest(html: string): Promise<boolean> {
    const $ = cheerio.load(html);
    if($("header.logo").length < 1) return false; // Check if the page contains an element with the class "logo" inside a header
    return true;
}